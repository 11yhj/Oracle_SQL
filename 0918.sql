...
-- GROUP BY 절을 통해 그룹화 된 결과 갑싀 번위를 제한할 때 사용
...
-- 조건에 맞는 사원 중에서 부서별, 직책별, 급여의 평균을 구하여 출력
SELECT DEPTNO, JOB, ROUND(AVG(SAL), 2)
FROM EMP
WHERE SAL >= 2000
GROUP BY DEPTNO, JOB
...
-- 1. 부서별 직책의 평균 급여가 500이상인 사원들의 부서 번호, 직책, 부서별 직책의 평균 급여 출력
...
-- 2. 부서번호, 평균급여, 최고급여, 최저급여, 사원수를 출력, 단, 평균급여를 출력 할 때는
-- 소수점 제회하고 부서 번호별 출력
SELECT DEPTNO 부서번호, TRUNC(AVG(SAL)) 평균급여, MAX(SAL) 최고급여
COUNTN(*) 사원수
FROM EMP
GROUP BY DEPTNO;

-- 3. 같은 직책에 종사하는 사원이 3명 이상인 직책과 인원을 출력
SELECT JOB 직책, COUNTN(*) 사원수
FROM EMP
GROUP BY JOB
HAVING COUNT(*) >=3;
...
-- ROLLUP : 명시한 일을 소그룹부터 대그룹의 순서로 각 그룹별 결과를
...
-- 조인 종류 : 두 개 이상의 테이블을 하나의 테이블처럼 가로로 늘려 출력하기 위해 사용하기
-- 조인은 대상 데이터를 어떻게 연결하느냐에 등가, 자체, 외부 조인으로 구분
-- 등가 조인 : 테이블으 연결한 후 출력 행을 각 테이블의 특정 열에 일치한 데이터를 기준으로 선정하는 방식
...
-- 외부 조인은 동등 조인인 조건을 만족하지 못해 누락되는 행을 출력하기 위해 사용
INSERT INTO EMP(EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO)
VALUES (9000, '장영원', 'SALESMAN', 7689, SYSDATE, 2000,1000, NULL);
...
-- JOIN ~ USING : 등가 조인을 대신해서 사용하기, USING 키워드에 조인 기준 열을 명시하여 사용
...
-- 사원 정보와 소속 부서 정보를 조회하는 경우에 대한 쿼리를 작성시
...
-- EXISTS 연산자 : 서브 쿼리의 결과 값이 하나 이상 존재하면 조건식이 모두 TRUE, 존재하지 않으면 모두 FALSE
SELECT *
FROM EMP
...
-- 다중 열 서브쿼리 : 서브쿼리의 결과가 두 개 이상의 컬럼으로 반환되어 메인 쿼리에 전달하는 쿼리
