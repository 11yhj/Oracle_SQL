...
SELECT JOB,
    SUBSTR(JOB, -LENGTH(JOB)), -- 음수는 뒤에서, 길이에 대한 음수값으로 역순 접근
...
SELECT LTRIM('zzzzzzzzzASDASDASDSAD         ', 'S')
FROM DUAL;

-- 날짜 데이터를 다루는 날짜 함수
-- SYSDATE : 운영체제의 햔재 날짜와 시간 정보를 가져 옴
SELECT SYSDATE FROM DUAL;

-- 날짜 데이터는 정수값으로 +, -가능(어제를 입력하고 싶으면 -1, 내일을 입력하고 싶으면 +1)
...
SELECT SYSDATE,
ADD_MONTHS(SYSDATE, 3) -- 두번째 인자는 달 수를 의미(3을 넣었으니 3개월 이후를 의미)
FROM DUAL;

-- EMP TABLE에서 입사 10주년이 되는 사원들 데이터 출력하기
SELECT EMPNO, ENAME, HIREDATE,
    ADD_MONTHS(HIREDATE, 120) AS 입사10주년
FROM EMP;

-- SYSDATE 와 ADD_MONTH 함수를 사용하여 현재 날짜와 6개월 후 날짜 출력
SELECT SYSDATE,
    ADD_MONTHS(SYSDATE, 6)
FROM DUAL;

-- 두 날짜 간의 개월 수 차이를 구하는 MONTHS_BETEEN 함수
...
SELECT EMPNO, ENAME, EMPNO + 'ABCD'  -- 이렇게 쓰면 에러가 뜬다.
FROM EMP
WHERE ENAME = 'FORD';

-- 날짜, 숫자를  문자로 변환하는 TO_CHRA 함수
-- 주로 날짜 데이터를 문자 데이터로 변환하는데 사용
-- 자바의 SimpleDataFormat() 유사
SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS') AS "현재 날짜 시간"  -- 글 띄어쓰기출력하고 싶으면 쌍따옴표 써야 한다 ""
FROM DUAL;
...
-- 다중행 함수 : 여러 행에 대한 함수가 적용되어 하나의 결과를 나타내는 함수(집계 함수)
-- 여러 행이 입력되어 결과가 하나의 행으로 출력됨
SELECT SUM(SAL)
FROM EMP;
